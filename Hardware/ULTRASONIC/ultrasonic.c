...
void Triger_PB3_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; //PB3 输入  
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; //IO口速度为50MHZ
	GPIO_Init(GPIOB, &GPIO_InitStructure);
	GPIO_ResetBits(GPIOB,GPIO_Pin_3);						 //PB3 下拉
...
{	 
  ...
  NVIC_InitTypeDef NVIC_InitStructure;
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD; //PB5 输入  
	GPIO_Init(GPIOB, &GPIO_InitStructure);
	GPIO_ResetBits(GPIOB,GPIO_Pin_5);						 //PB5 下拉
...
//	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD; //PB4输入  
//	GPIO_Init(GPIOB, &GPIO_InitStructure);
//	GPIO_ResetBits(GPIOB,GPIO_Pin_4);						 //PB4下拉
//	
//	//初始化定时器5 TIM5	 
//	TIM_TimeBaseStructure.TIM_Period = arr; //设定计数器自动重装值 
//	TIM_TimeBaseStructure.TIM_Prescaler =psc; 	//预分频器   
