...
void Flip_GPIO_One(void)
{
	...
	{  											      //每个定时器控制8路舵机运转，3个定时器控制24路舵机运转
		case 1: 
    	 ...
    	 GPIO_SetBits(GPIOB,GPIO_Pin_6);  //同时拉高控制舵机1的引脚的电
  	   flag_vpwm=1;	
				break;
		
		case 2:
   	    TIM2->ARR=MAXPWM-CPWM[1]; 		 //将2.5ms减去PWM脉宽值的数据赋值定时器2
				GPIO_ResetBits(GPIOB,GPIO_Pin_6);//同时拉低控制舵机1引脚的电平 
        flag_vpwm=1;	
